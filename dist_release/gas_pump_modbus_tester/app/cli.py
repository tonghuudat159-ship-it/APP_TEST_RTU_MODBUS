...
def build_parser() -> argparse.ArgumentParser:
    ...
    smoke_parser = subparsers.add_parser("smoke-test", help="run no-hardware simulator smoke test")
    ...
    smoke_parser.add_argument("--out", type=Path, default=Path("output/smoke_report.json"), help="JSON report path")
    smoke_parser.add_argument("--txt", type=Path, default=Path("output/smoke_report.txt"), help="TXT report path")
    smoke_parser.add_argument("--capture-jsonl", type=Path, default=Path("output/smoke_capture.jsonl"), help="raw capture JSONL output path")
    smoke_parser.add_argument("--capture-txt", type=Path, default=Path("output/smoke_capture.txt"), help="raw capture TXT output path")
...
def handle_version(args: argparse.Namespace) -> int:
    """Print application version details."""
...
def handle_smoke_test(args: argparse.Namespace) -> int:
    """Run simulator-only handoff smoke tests."""
    args.simulate = True
    args.port = "SIM"
    args.slave = DEFAULT_SLAVE_ID
    args.timeout = DEFAULT_TIMEOUT
    args.baudrate = DEFAULT_BAUDRATE
    args.debug = False
    capture = create_capture_from_args(args)
    ...
    try:
        client = GasPumpModbusClient(transport=transport, slave_id=args.slave)
        print("Gas Pump Modbus RTU Smoke Test")
        print("==============================")
        print()
        print(f"Simulator profile: {args.sim_profile}")
        print()

        _smoke_step(results, "ping", lambda: client.ping())
        _smoke_step(results, "status", lambda: client.read_quick_status())
        _smoke_step(results, "live", lambda: client.read_live_data())
        _smoke_step(results, "sensor", lambda: client.read_sensor_snapshot())
        _smoke_step(results, "clock", lambda: client.read_clock())
        _smoke_step(results, "log-latest", lambda: client.read_log(0))
        _smoke_step(results, "log-dump", lambda: client.read_all_logs(limit=3, include_invalid=True))
        ...
        def run_auto_test():
            ...
            if report.overall_status != "PASS":
                raise RuntimeError(f"auto-test overall status {report.overall_status}")
            return report

        _smoke_step(results, "auto-test", run_auto_test)
        _smoke_step(results, "config-unlock", lambda: client.unlock_config(1234))
        _smoke_step(
            results,
            "config-set-unit-price",
            lambda: client.set_unit_price(25000),
        )
        ...
        report_data = {
            "title": "Gas Pump Modbus RTU Smoke Test",
            "sim_profile": args.sim_profile,
            "overall": overall,
            "results": results,
        }
        try:
            write_json(args.out, report_data)
            Path(args.txt).parent.mkdir(parents=True, exist_ok=True)
            Path(args.txt).write_text(_format_smoke_report_txt(report_data), encoding="utf-8")
        except OSError as exc:
            print(f"ERROR: Failed to write smoke report: {exc}", file=sys.stderr)
            return 2
...
def _add_common_serial_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--config", type=Path, default=None, help="JSON config file with CLI defaults")
...
def _provided_option_names(argv: list[str]) -> set[str]:
    option_to_dest = {
        ...
        "--slave": "slave",
        "--debug": "debug",
        "--capture-txt": "capture_txt",
        "--capture-jsonl": "capture_jsonl",
        "--out": "out",
        "--txt": "txt",
    }
...
def _smoke_step(results: list[dict[str, str]], name: str, func) -> None:
    try:
        func()
    except Exception as exc:
        results.append(
...
def _format_smoke_report_txt(report_data: dict) -> str:
    lines = [
        report_data["title"],
        "=" * len(report_data["title"]),
        "",
        f"Simulator profile: {report_data['sim_profile']}",
        "",
    ]
    for result in report_data["results"]:
        lines.append(f"[{result['status']}] {result['name']} - {result['message']}")
    lines.extend(["", f"Overall: {report_data['overall']}", ""])
    return "\n".join(lines)
